...
typedef CanvasInteractiveViewerWidgetBuilder = Widget Function(
...
///
/// By default, CanvasInteractiveViewer clips its child using [Clip.hardEdge].
/// To prevent this behavior, consider setting [clipBehavior] to [Clip.none].
/// When [clipBehavior] is [Clip.none], CanvasInteractiveViewer may draw outside of
...
/// To prevent dead areas where CanvasInteractiveViewer does not receive gestures,
/// don't set [clipBehavior] or be sure that the CanvasInteractiveViewer widget is the
...
///     which includes the use of CanvasInteractiveViewer.
///   * The [flutter-go demo](https://github.com/justinmc/flutter-go), which includes robust positioning of an CanvasInteractiveViewer child
///     that works for all screen sizes and child sizes.
///   * The [Lazy Flutter Performance Session](https://www.youtube.com/watch?v=qax_nOpgz7E), which includes the use of an CanvasInteractiveViewer to
...
class CanvasInteractiveViewer extends StatefulWidget {
  /// Create an CanvasInteractiveViewer.
  ...
  /// Creates an CanvasInteractiveViewer for a child that is created on demand.
  ///
  ...
  CanvasInteractiveViewer.builder({
    ...
    this.transformationController,
    this.alignment,
    required CanvasInteractiveViewerWidgetBuilder this.builder,
  })  : assert(minScale > 0),
        assert(interactionEndFrictionCoefficient > 0),
        assert(minScale.isFinite),
        assert(maxScale > 0),
  ...
  /// If set to [Clip.none], the child may extend beyond the size of the CanvasInteractiveViewer,
  /// but it will not receive gestures in these areas.
  /// Be sure that the CanvasInteractiveViewer is the desired size when using [Clip.none].
  ///
  ...
  final CanvasInteractiveViewerWidgetBuilder? builder;

  /// The child [Widget] that is transformed by CanvasInteractiveViewer.
  ///
  ...
  /// is often useful when a child should be bigger than the CanvasInteractiveViewer.
  ///
  ...
  ///
  /// {@template flutter.widgets.CanvasInteractiveViewer.onInteractionEnd}
  ...
  ///
  /// A [GestureDetector] wrapping the CanvasInteractiveViewer will not respond to
  ...
  /// listeners are notified. If the value is set, CanvasInteractiveViewer will update
...
class _CanvasInteractiveViewerState extends State<CanvasInteractiveViewer>
  ...
  Rect get _boundaryRect {
    ...
    assert(
      ...
      "CanvasInteractiveViewer's child must have nonzero dimensions.",
    );
  ...
  void _receivedPointerSignal(PointerSignalEvent event) {
    ...
    if (event is PointerScrollEvent) {
      ...
      if (event.kind == PointerDeviceKind.trackpad && !pressedMeta) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.child != null) {
      child = _CanvasInteractiveViewerBuilt(
        childKey: _childKey,
      ...
      // When using CanvasInteractiveViewer.builder, then constrained is false and the
      ...
      child = LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          return _CanvasInteractiveViewerBuilt(
            childKey: _childKey,
            clipBehavior: widget.clipBehavior,
            constrained: widget.constrained,
            alignment: widget.alignment,
...
// CanvasInteractiveViewer's depending on if it's using a builder or a child.
class _CanvasInteractiveViewerBuilt extends StatelessWidget {
  const _CanvasInteractiveViewerBuilt({
    required this.child,
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside =
        CanvasInteractiveViewer.getNearestPointInside(point, boundary);
...
/// This enum is used to specify the behavior of the [CanvasInteractiveViewer] when
